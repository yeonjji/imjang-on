...
### L4 — 에디토리얼 링크 맥락화 (관련 가이드 + 관련 소식) [전 템플릿]
- **문제:** `BoardBriefingSection`이 카테고리 매칭 없이 모든 페이지에 동일한 최신 4건 노출. 게다가 승자들이 POI에서 링크하는 "관련 가이드"는 **상록 가이드**인데 imjang-on엔 그 대상 자체가 없음(board는 뉴스).
- **접근:** POI 상세에 **① 관련 가이드(L7 evergreen, 카테고리 1:1 매핑) — 승자 패턴의 핵심 링크** + **② 관련 소식(board 뉴스, 카테고리/지역 매칭)** 을 함께 노출. 가이드는 상록이라 항상 주제 적합, 소식은 시의성 보강. 매칭 부족 시에만 최신글 폴백.
...
### L7 — evergreen `/guide` 카테고리 가이드 신설 [오리지널 콘텐츠 앵커 · 승자 핵심]
- **문제:** 승자들이 POI에서 링크하는 "관련 가이드"는 **상록 카테고리 가이드**(ilsangkit "심야·공휴일 약국 찾기 완벽 가이드", ayo "부동산 법령 가이드"). imjang-on엔 이 콘텐츠 **타입 자체가 없음**(board는 뉴스 브리핑이고 가드레일이 해설·의견 금지). POI 142,700개가 링크할 고품질 상록 앵커가 부재.
...
- **Phase B — 콘텐츠 신호:** L7 · L3 · L5
  → **evergreen `/guide` 신설(승자 핵심 앵커, L4의 "관련 가이드" 링크 대상)** + FAQ/FAQPage 구조화 신호 + board 확대. 진짜 "데이터 풍부 콘텐츠" 레버. ※L4의 가이드 링크는 L7에 의존하므로, L7을 Phase B 선두에.
- **Phase C — 선택·운영:** L6
