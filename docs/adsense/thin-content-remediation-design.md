...
- **무게중심(중요):** 통과 필수 = **L1·L2(승자 공통) + 원본 콘텐츠 분량(L5/L7 존재)**. 나머지(L3·L4·L8)는 **ilsangkit식 polish — ayo는 없이 통과.** 특히 **에디토리얼 링크 축에서 imjang-on은 이미 통과 사이트(ayo)와 동일 상태**(둘 다 비맥락 전역 링크)라 **L4(맥락화)는 순수 품질 향상**이다.
...
- **Phase A — 통과 필수 · 코드만 (최대 볼륨):** L1 · L2
  → 병원 25% SSR 복구 + villa 35% 폴백. **둘 다 승자 공통(통과 필수급)** 이고 콘텐츠 생산 0. 색인 60%+ 를 코드로 개선.
- **Phase B — 차별화·polish (전부 ilsangkit식 · 통과 불필요, ayo는 없이 통과):** L7 · L3 · L4 · L5
  → evergreen `/guide` 신설 + FAQ/FAQPage + 에디토리얼 맥락화(L4) + board 확대. L4의 가이드 링크는 L7에 의존하므로 **L7을 선두에**. ※ imjang-on은 에디토리얼 링크 축에서 이미 통과 사이트(ayo)와 동일 상태라 **L4는 순수 품질 향상**(통과 게이트 아님).
- **Phase C — 선택·운영:** L6
