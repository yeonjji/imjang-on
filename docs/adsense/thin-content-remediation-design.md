...
- **무게중심(중요):** 통과 필수 = **L1·L2(승자 공통) + 원본 콘텐츠 분량(L7 `/guide` 신설로 확보, board 확대 아님)**. 나머지(L3·L4·L8)는 **ilsangkit식 polish — ayo는 없이 통과.** 특히 **에디토리얼 링크 축에서 imjang-on은 이미 통과 사이트(ayo)와 동일 상태**(둘 다 비맥락 전역 링크)라 **L4(맥락화)는 순수 품질 향상**이다.
...
> **근거 등급:** L1·L2 = **둘 다 하는 것(통과 필수, 코드)**. **L7 = 원본 콘텐츠 앵커**(둘 다 코퍼스 보유 → 분량 신호, **통과 관련**). L3·L4 = ilsangkit식 polish(ayo는 없이 통과). L5 = L4용 board 태깅(값싼 연동). L6 = ayo만(선택). → 통과 무게중심 = **L1·L2 + L7**, 나머지는 polish.
