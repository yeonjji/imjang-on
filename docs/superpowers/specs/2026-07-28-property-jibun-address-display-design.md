...
**좌표 없는 단지의 복사 버튼 부재 — 414단지 (0.2%)**

2026-07-28 PR #260에서 주소 줄을 `위치 · 로드뷰` 카드 **안**(지도 바로 위)으로 옮겼다. 그 카드는 `{coord && ...}` 안에 있으므로, 좌표가 없는 단지에서는 주소 줄 자체가 렌더되지 않는다.

| 유형 | 단지 수 | 좌표 없음 | 비율 |
|---|---:|---:|---:|
| APARTMENT | 45,476 | 221 | 0.5% |
| MULTIPLEX | 186,898 | 143 | 0.1% |
| ROW_HOUSE | 25,417 | 39 | 0.2% |
| OFFICETEL | 17,782 | 11 | 0.1% |
| **합계** | **275,573** | **414** | **0.2%** |

**실제 손실은 작다.** 그 페이지에서도 다음은 그대로 남는다:

- 주소 표시 — 히어로가 좌표와 무관하게 렌더한다 (`property-detail-hero.tsx:18,44`)
- 과장 방지 — 미확정이면 히어로가 `localityDisplay`(법정동까지)로 낮추므로 확정 주소를 주장하지 않는다
- JSON-LD — `streetAddress` 게이트는 좌표와 무관하게 작동한다
- 출처 표기 — 페이지 하단 `<MainSourceBlock id="molit-rtms" />`가 커버한다

빠지는 것은 **복사 버튼**과 `대표 지번` 배지·안내 문구다. 배지가 없어도 오도하지 않는 이유는, 그 자리에서 히어로가 애초에 지번을 주장하지 않기 때문이다 — 완화할 주장 자체가 없다.

**수정 방법(하게 되면):** 카드 조건을 `coord`에서 `coord || addr.street`으로 넓히고 `LocationViewer`만 `coord` 조건부로 남긴다. 제목은 지도가 없으면 `위치`로 낮춘다. 3개 페이지에 동일 적용.

**우선순위:** §7.2(동명 단지 병합, 10,762단지의 통계가 틀림)보다 두 자릿수 낮다. 편의 기능 부재이지 부정확이 아니다.
