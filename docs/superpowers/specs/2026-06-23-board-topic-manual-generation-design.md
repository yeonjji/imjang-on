...
5. **(신규) 저작권 안전** — 근거 본문은 **엄선된 공식 도메인 allowlist**의 공공저작물(근거: 저작권법 제24조의2 공공저작물 자유이용). **공공누리 명시적 제한 표시(제2·3·4유형)만 배제하고, 표시 없음(unknown)은 허용**(2026-06-24 완화 — §4 갱신 참조). 뉴스 기사 본문 비복제. 출처표시 필수. 신뢰 기준 = 도메인 allowlist + 뉴스 비복제 + 사람 검수.
...
> **갱신(2026-06-24):** 실운영에서 대부분의 .go.kr 안내 페이지(myhome·SH 등)에 공공누리 배지가 없어 전부 `unknown→배제`되며 기능이 막혔다. 공공누리는 자율 표시 제도일 뿐이고 **저작권법 제24조의2(공공저작물 자유이용)**가 배지 없는 공공저작물의 자유 이용을 허용하므로, 게이트를 **"명시적 제한(제2·3·4유형)만 배제, unknown은 공식 도메인 allowlist 신뢰 하에 허용"**으로 완화했다. 아래 원안의 'unknown 기본 배제' 표현은 이 갱신으로 대체된다.
...
| 2차 게이트(공공누리) | 추출 페이지에서 **공공누리 마커·유형 탐지** → **명시적 제한(제2·3·4유형)만 배제**, 제1유형·표시없음(unknown)은 허용 | unknown 허용 근거 = 저작권법 제24조의2. 2026-06-24 갱신 |
