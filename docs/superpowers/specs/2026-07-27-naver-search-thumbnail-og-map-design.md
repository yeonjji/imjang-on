...
- 의미 있는 지도를 만들 수 없으면 **그 문서 전용 이미지를 만들지 않는다** — 지도인 척하는 카드를 지어내지 않는다.
- 부정확한 위치를 정확한 것처럼 주장하지 않는다.

> **⚠️ 2026-07-27 실측 정정 — "이미지 없음"은 달성 불가능하다.**
> 최초 목표는 "지도를 못 만들면 og:image를 아예 내보내지 않는다"였다. **Next의 메타데이터 상속 때문에 그건 불가능하다.** 자체 `opengraph-image`가 없거나 `generateImageMetadata`가 `[]`를 반환하는 페이지는 **가장 가까운 조상의 이미지를 상속**한다. 루트에 `app/opengraph-image.tsx`가 있는 한 사이트의 모든 페이지가 그걸 물려받는다.
>
> 실측(dev, 2026-07-27): `/board`·`/list`·`/apt` 전부 `og:image = /opengraph-image`, `alt = "임장ON — 공공데이터 부동산 실거래가"`를 방출했다. 좌표 없는 매물 상세도 동일하다.
>
> **결정 (사용자):** 루트 카드를 유지한다. 8절의 `OgFrame` 중앙정렬 재디자인 덕에 상속된 카드는 정사각 크롭 후에도 읽히므로, 애초 문제였던 **빈 파란 사각형이 아니다**. 진짜 "무이미지"를 원하면 `app/opengraph-image.tsx`를 지워야 하는데, 그러면 도메인 루트의 카톡·SNS 공유 미리보기까지 사라진다 — 그 대가는 받지 않기로 했다.
>
> 따라서 이 작업의 실제 성과는 "무이미지"가 아니라 **"문서마다 지도, 아니면 읽히는 브랜드 카드"** 다. 어느 쪽이든 빈 사각형은 사라진다.
...
- **`/board/[id]`, `/finance/[seq]`** — `opengraph-image.tsx` 파일 **삭제**. 조건 분기가 필요 없다.
  ...
  > **게시판 JSON-LD 이미지는 유지한다 (2026-07-27 결정).** 구현 중 발견: `/board/[id]`는 `articleSchema.image`로 별도 라우트 `/board/{id}/thumbnail`(= `OgFrame` 렌더)을 광고한다. 최초 계획은 이 라우트를 몰랐다.
  >
  > og:image만 지우고 이걸 남기면 파란 카드가 JSON-LD를 통해 계속 노출될 수 있다 — 그런데 **8절의 `OgFrame` 중앙정렬 재디자인이 이 전제를 바꿨다.** 이제 정사각 크롭 후에도 글 제목이 읽히므로, 문제였던 "빈 파란 사각형"이 아니라 **읽히는 제목 카드**다.
  >
  > 따라서 그대로 둔다. 지도가 없는 문서에 지도를 지어내지 않으면서도, Google Article 구조화 데이터의 권장 필드 `image`를 잃지 않는다. `/board/{id}/thumbnail` 라우트와 `articleSchema.image`는 이번 작업 범위 밖이다.
