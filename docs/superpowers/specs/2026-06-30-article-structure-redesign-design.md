...
- **핵심 요약 콜아웃**:
  ...
  - 상세 페이지가 `rest`를 일반 `.board-prose`로 렌더한 뒤, `summary`가 있으면 콜아웃
    컴포넌트(`<aside>`, 부드러운 배경·옅은 보더, 추가 그림자 없음)를 **본문 아래·출처 위**에 렌더.
    (생성·분리는 마크다운 맨 앞 `## 핵심 요약` 기준 유지 — 저장 위치는 맨 위, 표시 위치만 하단.)
