...
## 운영 실측 기록 (Task 7 · 2026-09-14)

머지·배포 후 운영에서 확인했다. 전부 읽기 전용. 배포 커밋 `dabb9ec`(PR #302 포함).

### 1·2. 렌더 확인 — 통과

| 대상 | 단지정보 섹션 | 앵커 `#complex` | 사이드바 항목 | 구성 줄 | "정보 없음" |
|---|---|---|---|---|---|
| 매칭 3건 (66264·234020·233838) | ✅ | ✅ | ✅ | ✅ | 없음 |
| 미매칭 3건 (20213·244537·20257) | 안 뜸 | 안 뜸 | 안 뜸 | 안 뜸 | 없음 |

사이드바 항목과 본문 섹션이 함께 뜨고 함께 사라진다. 빈 카드도 대체 문구도 새어나오지 않는다.

### 3. 색인 계약 — 통과

표본 1,000. **현재 색인 대상 870 = 87.0%**로 배포 전 기준(86.8~87.0%)과 같다.

모듈별 발화: `scale 870 · peer 870 · access 865 · trend 787 · flags 380 · floor 90`.
**`unitMix`·`parking`이 목록에 없다** — 단지 모듈이 `fired`에 새어 들어가지 않았음을 운영 표본으로 확인.

### 4. 최종 리뷰 지정 항목

**준공연도 출처 이원화 — 92 / 12,518 (0.7%)**
`Property.builtYear`(실거래 신고 건축년도)와 `AptComplex.usedate`(사용승인일) 연도가 다른 단지. 예: 세종마루투 신고 2018 vs 승인 2015. 한 페이지에 두 연도가 같이 뜬다. 0.7%라 지금 막을 사안은 아니지만 표기 통일은 후속 과제.

**`dominant.pct` 구간 — 임계 수정이 의도대로 동작**

| 구간 | 단지 | 결과 |
|---|---|---|
| 100% | 5,008 | "한 종류" 발화 |
| 90~99% | 595 | 이번 수정으로 "한 종류"에서 빠짐 |
| 90% 미만 | 6,335 | — |

90~99% 표본 3건 전부 `한 종류` 문구 없이 다음 분기가 발화했고, 산문 수치와 구성 바가 일치했다. 분기 공백 0건.

```
/apt/233524  산문 "85㎡ 초과가 100%로 중대형 비중이 높은"  구성 85~135㎡ 91% · 135㎡ 초과 9%
/apt/232642  산문 "85㎡ 이하가 97%인 중소형 중심"          구성 60㎡ 이하 96% · 60~85㎡ 1% · 85~135㎡ 2% · 135㎡ 초과 1%
/apt/233598  산문 "85㎡ 이하가 100%인 중소형 중심"         구성 60㎡ 이하 93% · 60~85㎡ 7%
```

**매매 거래가 없는 매칭 단지 — 796 / 11,938 (6.7%)**
「면적별 실거래 비교」 카드에 실거래 행이 하나도 없이 구성 줄만 뜬다. 설계 의도(공급 구성은 거래가 없어도 사실이다)지만 **제목과 내용이 어긋난다.** 실제 렌더:
...
후속 판단 필요 — 제목을 조건부로 바꾸거나, 거래가 없을 때 구성 줄을 「단지 정보」로 옮기는 선택지가 있다.
