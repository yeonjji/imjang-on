...
## 9. 실행 기록 (2026-09-08 ~ 09, 운영 반영 완료)

### 9.1 결과

| | 값 |
|---|---|
| `AptComplex` 적재 | **22,301건** (상세 100%, `rawJson` 전량 보존) |
| 매칭 확정 | **12,544 (56%)** — Tier1 11,636 · Tier2 908 |
| 미매칭 | 9,757 (44%) |
| `Property` 커버리지 | 28% |
| `Property.households` 역채움 | **11,938** (= 12,544 − 세대수 결측 606) |
| `Property.buildingCount` 역채움 | **12,531** (= 12,544 − 결측 13) |
| 면적 4칸 완비 + 합 일치 | 21,422 (96%) |

audit(읽기 전용) 예측과 실행 결과가 **한 건도 어긋나지 않았다.** 같은 코드 경로를 쓰게 한 설계가 의도대로 동작했다.

### 9.2 측정 게이트 — 통과

`builtYear` 결측 **0/44,481** → `bScale`은 이미 100% 발화 중이므로 역채움으로 `fired`가 늘어난 페이지는 **0건**.

표본 관측도 같은 답을 냈다. 색인 대상 비율이 매칭 전 86.8%(n=2,500) → 매칭 후 87.0%(n=700)로 사실상 동일하고, 모듈별 발화율(`scale`·`peer` 100%, `trend` 89%, `flags` 43%, `floor` 9%)도 변하지 않았다.

### 9.3 채움률 — §1.2의 36건 표본을 전수로 대체

| 필드 | 전수 22,301 | 36건 표본 |
|---|---|---|
| 동수·사용승인일·복도유형·EV | **100%** | 97~100% |
| 최고층·시공사 | 99% | 100%·97% |
| **면적별 4칸 + 합 일치** | **96%** | **61%** |
| 세대수·지하주차·CCTV | 96% | 86%·69%·89% |
| 승강기 | 92% | 미측정 |
| 지하철역 | **19%** | 50% |

**면적별 세대 구성이 61%가 아니라 96%다.** §1.2의 "전체 아파트로 환산하면 30%"는 틀렸다. 화면 스펙에서 주택 구성 분석을 주력으로 삼을 수 있다.

지하철은 19%로 낮지만 우리는 `SubwayStation`으로 최근접 역·실거리를 이미 계산하므로 이 필드는 쓰지 않아도 된다.

### 9.4 운영에서만 드러난 결함 5건

**HTTP 429 — 초당 요청 제한.** 워커 4 × 레코드내 `Promise.all` 2 = 동시 8요청에서 다발했다. 레코드 안을 순차로 바꾸니 300건 23초에 skip 0. 한 번 429를 맞으면 페널티 창이 이어져 뒤따르는 요청도 죽는다 — 동시성 1 측정이 skip 13으로 나온 건 그 순서 효과였다.

**0을 결측으로 쓴다.** 세대수·동수·최고층이 `0`인 빈 레코드가 있다(디마크당산 A10019936). 그대로 두면 "0세대 단지입니다"가 렌더된다. 주차·EV·면적대의 0은 실제 값이라 살린다.

**`subwayLine`에 전국 노선 목록.** 32건이 컬럼 길이 초과로 적재 실패했다. 서울 송파 단지에 부산-김해경전철이 붙는다. 역명이 쉼표뿐이면 노선도 버리고, 노선 5개 초과면 전국 목록으로 본다(최다 환승 김포공항역 5개).

**동명 접두를 한쪽만 벗기면 안 된다.** 우리 이름에도 붙는다("범어에일린의뜰" vs API "에일린의뜰"). 양쪽 두 형태를 교차 비교해야 한다. §5.1의 "우리 이름엔 거의 없다"는 틀렸다.

**⚠️ Tier1에도 동명 게이트가 필요하다 — §5.2 정정.** 동명 접두 제거 자체가 오매칭을 만든다. "풍납동신동아" → "신동아"로 벗기면 다른 동의 흔한 이름과 완전일치한다. Tier1 11,296건 중 **342건(3%)** 이 이 경로였다("숭의현대"(숭의동) → "현대"(주안동) 식). Tier1에 게이트를 걸자 불일치가 0이 됐고, 동시에 게이트가 모호한 복수 후보를 갈라 주어 Tier1이 11,296 → 11,642로 늘었다.

**차수 게이트 추가.** 동명이 같아도 차수가 다르면 다른 단지다("동탄파라곤2" → "동탄파라곤"). 로마숫자를 정규화해 "…파크 1" ↔ "…파크ⅰ"는 통과시킨다.

임계값 0.85는 유지했다. Tier2 표본 70건 육안 검수에서 의심 1건(1.4%).

### 9.5 후속 스펙에 넘기는 실측

화이트리스트 전환 시 **아파트 색인 페이지의 55%가 빠진다**(무작위 표본 2,500). 운영 규모로 약 21,000페이지다. 처음 400건(id 오름차순) 표본에서 33%로 나왔던 것은 편향이었다 — 먼저 적재된 지역은 거래가 많아 `trend`·`flags` 발화율이 높다.
