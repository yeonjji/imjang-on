...
const CANONICAL: Array<[RegExp, string]> = [
  // 임장ON 자체집계라도 '청약' 집계일 때만 이 라벨을 쓴다. `/임장ON/`처럼 넓게 잡으면
  // '임장ON 실거래 집계 · 국토교통부 실거래가 공개시스템'(sourceUrl=rt.molit.go.kr)까지
  // '청약 집계'로 표시돼 화면 기관명과 링크 대상이 어긋난다. `[^·]*`는 `·`로 구분된
  // 다른 기관 이름의 '청약'에 걸려 넘어가지 않게 하는 경계다.
  [/임장ON[^·]*청약/, '임장ON 청약 집계'],
  ...
  [/세종특별자치시|sejong\.go\.kr/i, '세종특별자치시'],
  [/균형발전종합정보시스템|nabis\.go\.kr/i, '균형발전종합정보시스템'],
  [/국토연구원/, '국토연구원'],
