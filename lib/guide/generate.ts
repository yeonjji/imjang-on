...
8. '## 핵심 요약' 바로 다음에는 곧장 첫 번째 본문 '## 소제목' 섹션이 온다(그 사이에 소제목 없는 문단을 두지 않는다). '이런 분께 필요한 정보'는 그 첫 섹션의 도입 문장에 자연스럽게 한두 문장으로 녹인다(별도의 '필요한 사람' 소제목은 만들지 않는다).
9. 본문을 2~3개의 '## 소제목' 섹션으로 나눈다(개념 → 확인·이용 방법 → 유의점 흐름). 소제목 문구는 내용에 맞게 자유롭게 붙인다.
10. '## 자주 묻는 질문' 섹션에 질문 3~5개를 '**Q. 질문?** A. 답변' 형식으로 쓴다.
11. '## 더 알아보기' 섹션에 "조건은 달라질 수 있으니 공식 공고·자료를 함께 확인하는 것이 좋습니다" 취지의 안내 문장을 넣고, 이어서 사용자 메시지로 제공된 CTA 링크를 '관련 정보 확인하기 → [라벨](경로)' 형태로 그대로 이 위치에 넣는다.
