...
const load = cache(async ({ id }: { sigunguCode: string; id: string }) => {
  ...
  const pharmacy = await getPharmacyById(pharmacyId).catch(() => null);
  if (!pharmacy) return null;
  // 시설·청약은 원본 공공데이터에 좌표가 실려 오므로 지역 폴백을 두지 않는다.
  const coord = await getMapEntityLatLng('pharmacy', pharmacyId);
  if (!coord) return null;
  const title = pharmacy.name;
  const subtitle = '약국 · 주변 아파트 실거래가';
  return {
    title,
    subtitle,
    alt: `${title} 위치 지도`,
