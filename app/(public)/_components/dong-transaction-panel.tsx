 ...
 *
 * 시도 드롭다운은 getSidoList()의 정적 17개 상수라 Region에 행이 없는 시도까지 항상
 * 뜬다. 운영 읽기전용 프로브 실측(2026-09): 18개 중 광주·전남 2개가 /api/regions에서
 * 빈 시군구 목록을 낸다(전남광주통합특별시는 27개로 정상 — 2026-07-01 행정구역
 * 개편으로 광주·전남이 여기로 흡수됐는데 lib/region.ts의 정적 상수엔 옛 두 항목이
 * 그대로 남아 있다). 18개 중 2개, 11%가 실제로 이 경로를 탄다 — 가정이 아니다.
 * 그런 시도를 고르면 /api/regions가 빈 배열을 주거나(실측상 이 경우) 실패하고,
