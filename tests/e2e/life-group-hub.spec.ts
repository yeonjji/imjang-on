...
test.describe('생활편의 그룹 허브 /life/[group]', () => {
  test('education: 학교/어린이집만 노출, 다른 그룹 카드 없음', async ({ page }) => {
    ...
    await expect(main.getByRole('link', { name: /🏫\s*학교/ }).or(main.getByRole('link', { name: '학교', exact: true }))).toBeVisible();
    await expect(main.getByRole('button', { name: /어린이집/ }).or(main.getByRole('link', { name: /어린이집/ }))).toBeVisible();
    // 다른 그룹 카드는 없어야 함 — 카드 그리드만 스코프
    const grid = main.locator('.grid');
  ...
  test('amenity: 편의점/마트/카페/전통시장 4개만 노출', async ({ page }) => {
    ...
    const grid = page.getByRole('main').locator('.grid');
